...
if bCosine && ~options.bNormalized
    Normfea = NormalizeFea(fea); %若未正则
end

if strcmpi(options.NeighborMode,'KNN') && (options.k > 0)
    if ~(bCosine && options.bNormalized)   % 非cosine
        G = zeros(nSmp*(options.k+1),3); % 初始化 3个
        ...
        G = zeros(nSmp*(options.k+1),3);
        for i = 1:ceil(nSmp/BlockSize)
            a = ceil(nSmp/BlockSize);
            if i == ceil(nSmp/BlockSize)
                smpIdx = (i-1)*BlockSize+1:nSmp;
                dist = fea(smpIdx,:)*fea'; %距离计算
                dist = full(dist);
                ...
                if bSpeed
                    ...
                    idx = dump;
                    for j = 1:options.k+1
                        [dump(:,j),idx(:,j)] = max(dist,[],2); %以最大余弦相似度为k个最近邻居
                        temp = (idx(:,j)-1)*nSmpNow+[1:nSmpNow]';
                        dist(temp) = 0;
                    end
                else
                    [dump idx] = sort(-dist,2); % sort each row
                    idx = idx(:,1:options.k+1); % k个近邻 k+1是因为除了本身 有可能本身不是最大的
                    dump = -dump(:,1:options.k+1); % dump可直接作为权重
                end

                G((i-1)*BlockSize*(options.k+1)+1:nSmp*(options.k+1),1) = repmat(smpIdx',[options.k+1,1]); %复制，行复制k+1次，列复制一次
                
                G((i-1)*BlockSize*(options.k+1)+1:nSmp*(options.k+1),2) = idx(:); %最近的邻居 1:n、n+1:2n、2n+1:3n、3n+1:4n;
                G((i-1)*BlockSize*(options.k+1)+1:nSmp*(options.k+1),3) = dump(:); %相似度权重 赋权
            else
                smpIdx = (i-1)*BlockSize+1:i*BlockSize;
                dist = fea(smpIdx,:)*fea';
                dist = full(dist);
                ...
                G((i-1)*BlockSize*(options.k+1)+1:i*BlockSize*(options.k+1),1) = repmat(smpIdx',[options.k+1,1]);
                G((i-1)*BlockSize*(options.k+1)+1:i*BlockSize*(options.k+1),2) = idx(:);
                G((i-1)*BlockSize*(options.k+1)+1:i*BlockSize*(options.k+1),3) = dump(:);  % 权重
            end
        end
...
switch lower(options.WeightMode)  %非knn
    case {lower('Binary')}
